...
type ConvergedResponse struct {
	...
	AuthMethodInputFieldName string             `json:"sAuthMethodInputFieldName"`
}
...
func (ac *Client) processMfaAuth(mfaResp mfaResponse, convergedResponse *ConvergedResponse) (*http.Response, error) {
	...
	if convergedResponse.AuthMethodInputFieldName != "" {
		if convergedResponse.AuthMethodInputFieldName == convergedResponse.SFTName ||
			convergedResponse.AuthMethodInputFieldName == "request" ||
			convergedResponse.AuthMethodInputFieldName == "login" ||
			convergedResponse.AuthMethodInputFieldName == "canary" ||
			convergedResponse.AuthMethodInputFieldName == "hpgrequestid" {
			return res, errors.New("MFA auth method input field conflicts with ProcessAuth field")
		}
		formValues.Set(convergedResponse.AuthMethodInputFieldName, mfaResp.AuthMethodID)
	}
	if convergedResponse.APICanary != "" {
		formValues.Set("canary", convergedResponse.APICanary)
	}
	if convergedResponse.SessionID != "" {
		formValues.Set("hpgrequestid", convergedResponse.SessionID)
	}
